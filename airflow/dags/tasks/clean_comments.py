...
@task()
def clean_comments(json_comments):
    comments = json.loads(
        json_comments, object_hook=lambda d: SimpleNamespace(**d))
    for comment in comments:
        comment.cleaned_content = process_review(comment.content)
    clean_json_comments = json.dumps([ob.__dict__ for ob in comments])
    return clean_json_comments
