...
def fetch_comments():
  ...
  for comment_json in comments_json:
    ...
    comment = Comment(id, video_id, content, author_name, author_profile_image_url, like_count, published_at, updated_at)
    comments.append(comment)

  df = pd.DataFrame([c.__dict__ for c in comments ])
  
  # df.to_csv('comments.csv')
  
  print(json.dumps(comments[0].__dict__))
